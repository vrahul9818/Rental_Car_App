...
const AdminCarEditDelete = () => {
  ...
  const handelDelete = () => {
    console.log(uid);
    axios
      .delete("http://localhost:8080/car_rent/adminCarData/Delete", {
        data: { uid: uid },
      })
      .then((response) => {
        console.log(response.data, "response axios");
    ...
    <>
      ...
      <div className='AdminCarDisplay'>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            id='adminCarEditADD'
            className='adminCarEditADDcl'
            type='submit'>
            Edit
          </button>
        </form>
        <button onClick={handelDelete} id='adminCarEditDelete' type='delete'>
          Delete
        </button>
      </div>
    </>
